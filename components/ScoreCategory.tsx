...
export const ScoreCategory: React.FC<ScoreCategoryProps> = ({
  ...
  return (
    <tr className={`border-b ${
      ...
      {players.map((_, playerIndexInner) => {
        const playerScoreData = scores?.[playerIndexInner]?.[categoryIndex];
        const canSelect = playerIndexInner === currentPlayer && 
                          rollCount > 0 && 
                          possibleScores?.[categoryIndex] !== null && 
                          possibleScores?.[categoryIndex] !== undefined &&
                          playerScoreData &&
                          !playerScoreData.locked;

        return (
          <td key={playerIndexInner} className={`p-2 sm:p-4 text-center ${playerIndexInner === currentPlayer ? isJerryGame ? 'bg-gray-700' : 'bg-red-50' : ''}`}>
